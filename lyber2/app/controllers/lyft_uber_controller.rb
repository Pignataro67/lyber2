class LyftUberController < ApplicationController
  ...
  def uber_estimate
    ...
    drop_off_lat = params[:drop_off_lat]
    drop_off_long = params[:drop_off_long]
    res = RestClient.get("https://api.uber.com/v1/cost?start_lat=#{startlat}&start_lng=#{start_long}&end_lat=#{drop_off_long}", headers={:Authorization => "bearer #{ENV['UBER_ESTIMATE']}"})     render json: res
  end
end